...
from models.commons.layers import LayerNorm, Embedding
...
class EncSALayer_cond(nn.Module):
    def __init__(
        self,
        c,
        num_heads,
    ...
    def forward(self, x, encoder_padding_mask=None, emo_cond=None, spk_cond=None, **kwargs):
        layer_norm_training = kwargs.get("layer_norm_training", None)
        if layer_norm_training is not None:
            self.layer_norm1.training = layer_norm_training
            self.layer_norm2.training = layer_norm_training
        if self.num_heads > 0:
            residual = x
            x = self.layer_norm1(x)
            emo_cond = emo_cond.transpose(0,1)
            spk_cond = spk_cond.transpose(0,1)
            emo_x = x + emo_cond
            spk_x = x + spk_cond
            ...
            (
                ...
                query=emo_x, key=emo_x, value=emo_x, key_padding_mask=encoder_padding_mask
            )
            (
                spk_x,
                _,
            ) = self.self_attn(
                query=spk_x, key=spk_x, value=spk_x, key_padding_mask=encoder_padding_mask
            )
            
            # emo_x = F.dropout(emo_x, self.dropout, training=self.training)
            # spk_x = F.dropout(spk_x, self.dropout, training=self.training)
            x = residual + emo_x + spk_x
            x = x * (1 - encoder_padding_mask.float()).transpose(0, 1)[..., None]
...
class TransformerEncoderLayer_cond(nn.Module):
    def __init__(self, hidden_size, dropout, kernel_size=9, num_heads=2):
        super().__init__()
        self.hidden_size = hidden_size
        self.dropout = dropout
        self.num_heads = num_heads
        self.op = EncSALayer_cond(
            hidden_size,
            num_heads,
            dropout=dropout,
            attention_dropout=0.0,
    ...
    def forward(self, x, emo_cond=None, spk_cond=None, **kwargs):
        return self.op(x, emo_cond=emo_cond, spk_cond=spk_cond, **kwargs)
...
class FFTBlocks_encoder(nn.Module):
    def __init__(
        self,
        hidden_size,
        num_layers,
        ...
        self.layers1 = TransformerEncoderLayer(
                    self.hidden_size,
                    self.dropout,
                    kernel_size=ffn_kernel_size,
                    num_heads=num_heads,
                )
        self.layers2 = TransformerEncoderLayer(
                    self.hidden_size,
                    self.dropout,
                    kernel_size=ffn_kernel_size,
                    num_heads=num_heads,
                )
        self.layers3 = TransformerEncoderLayer_cond(
                    self.hidden_size,
                    self.dropout,
                    kernel_size=ffn_kernel_size,
                    num_heads=num_heads,
                )
        self.layers4 = TransformerEncoderLayer(
                    self.hidden_size,
                    self.dropout,
                    kernel_size=ffn_kernel_size,
                    num_heads=num_heads,
    ...
    def forward(self, x, padding_mask=None, attn_mask=None, emo_cond=None, spk_cond=None, return_hiddens=False):
        """
        :param x: [B, T, C]
        :param padding_mask: [B, T]
        :return: [B, T, C] or [L, B, T, C]
        ...
        # layers1
        x = (
            self.layers1(x, encoder_padding_mask=padding_mask, attn_mask=attn_mask)
            * nonpadding_mask_TB
        )
        hiddens.append(x)
        ...
        # layers2
        x = (
            self.layers2(x, encoder_padding_mask=padding_mask, attn_mask=attn_mask)
            * nonpadding_mask_TB
        )
        hiddens.append(x)
        ...
        # layers3
        x = (
            self.layers3(x, encoder_padding_mask=padding_mask, attn_mask=attn_mask,
            emo_cond=emo_cond, spk_cond=spk_cond)
            * nonpadding_mask_TB
        )
        hiddens.append(x)
        ...
        # layers4
        x = (
            self.layers4(x, encoder_padding_mask=padding_mask, attn_mask=attn_mask)
            * nonpadding_mask_TB
        )
        hiddens.append(x)
        if self.use_last_norm:
...
class FastSpeechEncoder(FFTBlocks_encoder):
    def __init__(
        self,
        dict_size,
        hidden_size=256,
    ...
    def forward(self, txt_tokens, attn_mask=None, emo_cond=None, spk_cond=None):
        """
        ...
        if self.num_layers > 0:
            x = super(FastSpeechEncoder, self).forward(
                x, encoder_padding_mask, attn_mask=attn_mask, emo_cond=emo_cond, spk_cond=spk_cond
            )
        return x
...
class FastSpeechDecoder(FFTBlocks_encoder):
